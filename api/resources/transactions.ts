...
export function registerTransactions(router: Router): void {
  ...
  router.add('POST', '/expenses', withAuth('user', async (req, res) => {
    ...
    const name = String((body.name || '').toString().trim()) || 'Transaction';
    ...
    try { await repo.logActivity(userId, budgetId, 'add-transaction', 'transaction', t.id, { name, total, type }); } catch {}
